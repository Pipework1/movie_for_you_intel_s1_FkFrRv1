...
class Exam(QWidget, form_window):
    def __init__(self):
        ...
        self.df_reviews = pd.read_csv('./Data_In_Lecture_1/cleaned_one_review.csv')
    ...
    def combobox_slot(self):        # 슬롯 정의
        title = self.comboBox.currentText()
        recommendation = self.recommendation_by_movie_title(title)
        self.lbl_recommendation.setText(recommendation)

    def recommendation_by_movie_title(self, title):     # 영화 추천
    ...
    def getRecommendation(self, cosine_sim):        # 추천 영화 리스트
...
if __name__ == '__main__':
    ...
    mainWindow = Exam()
    mainWindow.show()
    sys.exit(app.exec_())   #exec_() : 시그널을 받아서 슬롯에 전달해주는 함수.